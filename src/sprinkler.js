...
(function (root, factory) {
  ...
  var Sprinkler = function (canvas) {
    ...
    var tickModel = function (dt) {
      ...
      // Update model: remove particles that are out of screen
    ...
    var imagesToParticles = function (images) {
      // Turn images to particles
      var i, img, p, newParticles = [], w, h;
      w = canvas.width;
      h = canvas.height
      ...
      for (i = 0; i < images.length; i += 1) {
        ...
        p = new Particle(
          ...
          -Math.max(img.width, img.height) / 2, // maxRadius above canvas top
          1, // z, scale
    ...
    this.load = function (imagePaths, callback) {
      ...
      //   callback
      //     function (start, stop)
      loadImages(imagePaths, function (err, images) {
        if (err) { console.error(err); return; }
        particles = imagesToParticles(images);
        callback(start, stop);
      });
    };
  };
